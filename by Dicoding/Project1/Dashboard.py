...
def create_daily_orders_df(df):
    ...
    daily_order_df = daily_orders_df.reset_index()
...
def create_sum_order_items_df(df):
    sum_orders_items = df.groupby(by = "product_name").quantity_x.sum().sort_values(ascending = False).reset_index()
    return sum_orders_items
...
all_df = pd.read_csv("all data.csv")
...
with st.sidebar:
    ...
    start_date, end_date = st.date_input(
        label='Rentang Waktu',min_value=min_value,
        max_value=max_value,
...
byage_df = create_by_age_df(main_df)
bystate_df = create_bystate_df(main_df)
rfm_df = create_rfm_df(main_df)

st.header('Dicoding Collection Dashboard :sparkles:')
st.subheader('Daily Oreders')
...
st.caption('Copyright (c) Barudak KTT 2023')